...
%% 最优参数
alpha_opt = 0.6 
lambda_opt = 43.287612810830602
%% 约束条件
theta_0 = zeros(q,1); % 初值为0
u_0 = (1e-6)*ones(q,1);  % 初值为1
...
%% 保存
csvwrite(['theta_hatopt55example.csv'],theta_hat);
csvwrite(['thetaopt55example.csv'],theta);
csvwrite(['P_testopt55example.csv'],P_test);
csvwrite(['paraopt55example.csv'],para);
% end
